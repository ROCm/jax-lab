...
# Opt out of the new `jax.pmap` implementation
# due to performance regression in JAX 0.8.0
os.environ["JAX_PMAP_SHMAP_MERGE"] = "0"
...
class TrainingArguments:
    ...
    use_train_set_for_eval: bool = field(
        default=False, metadata={"help": "Whether to use train dataset for eval"}
    )
    batch_num: int = field(
        default=100, metadata={"help": "Number of batches for evaluation"}
    )
...
class ModelArguments:
    ...
    use_auth_token: bool = field(
        ...
        metadata={
            "help": (
                "Will use the token generated when running `hf login` (necessary to use this script "
                "with private models)."
            )
        },
    )
...
def main():
    ...
    if training_args.do_eval:
        if training_args.use_train_set_for_eval:
            num_eval_samples = len(tokenized_datasets["train"])
            eval_samples_idx = np.arange(
                training_args.batch_num
        ...
        for i, batch_idx in enumerate(
            ...
            mertics = jax.device_get(metrics)
            eval_metrics.append(metrics)
            if i > 2:  ##discard performance numbers of 2 warm-up batches
                eval_times.append(time.time() - start)
        print("eval_batches,", len(eval_batch_idx))
        print("eval_times,", sum(eval_times) / len(eval_times))
