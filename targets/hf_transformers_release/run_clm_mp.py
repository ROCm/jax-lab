...
if LooseVersion(jax.__version__) >= LooseVersion("0.4.6"):
...
class DataTrainingArguments:
    ...
    batch_num: Optional[int] = field(
        default=100,
        metadata={"help": ("Total batches for evaluation" "value if set.")},
    )
    use_train_set_for_eval: bool = field(
        default=False,
        metadata={"help": "Use train dataset split for performance evaluation"},
    )
...
def main():
    ...
    if training_args.do_eval:
        ...
        if data_args.use_train_set_for_eval:
    ...
    p_eval_step = pjit(
        eval_step,
        in_shardings=(None, None, param_spec),
        out_shardings=None,
    )
    ...
    with Mesh(mesh_devices, ("dp", "mp")):
        ...
        if training_args.do_eval:
            ...
            rng, input_rng = jax.random.split(rng)
            eval_loader = data_loader(input_rng, eval_dataset, eval_batch_size)
            eval_steps = min(data_args.batch_num, len(eval_dataset) // eval_batch_size)
            eval_times = []

            for i, _ in enumerate(
                tqdm(range(eval_steps), desc="Evaluating...", position=2, leave=False)
            ):
                batch = next(eval_loader)
                start = time.time()
            ...
            logger.info(
                f"Eval loss: {eval_metrics['loss']} | Eval Perplexity:"
                f" {eval_metrics['perplexity']}"
            )
